...
--
-- 改了什麼：
--   貼文一發出來就同時通知 IG 和 Threads，不再等排程（幾秒內上架）
--   排程從每 5 分鐘改成每分鐘，降級成補救網
--
-- ※ IG 沒有同步刪除功能（要走 Facebook 登入那套，跟本站的 Instagram 登入
--   不相容）。即時同步代表違規貼文會先上 IG，檢舉自動隱藏來不及攔，
--   事後只能人工到 IG 上刪。這是刻意選擇的取捨。
-- ============================================================

-- 新貼文一寫進資料庫就立刻通知兩個發佈端點。
...
-- 兩個排程改成每分鐘跑一次。
-- 它們現在只是補救網：撿走 trigger 沒送到、或當下發佈失敗的貼文。
...
--
-- 只想讓 IG 退回「等 3 分鐘緩衝」、Threads 維持即時：
--   把上面 function 裡呼叫 ig-publish 的那段 perform 刪掉，重新執行
-- ============================================================

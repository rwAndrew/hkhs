...
export async function fetchPost(id) {
  ...
  const res = await fetch(
    `${SUPABASE_URL}/rest/v1/posts?id=eq.${encodeURIComponent(id)}&hidden=eq.false&select=id,board,anon_emoji,anon_name,title,body,likes,created_at`,
...
// 貼文頁要把留言一起印進 HTML，搜尋引擎才看得到討論內容
export async function fetchComments(id, limit = 50) {
  if (!SUPABASE_URL || !SUPABASE_ANON_KEY || !id) return [];
  const res = await fetch(
    `${SUPABASE_URL}/rest/v1/comments?post_id=eq.${encodeURIComponent(id)}&hidden=eq.false` +
    `&order=created_at.asc&limit=${limit}&select=id,body,anon_emoji,anon_name,created_at`,
    { headers: { apikey: SUPABASE_ANON_KEY } }
  );
  if (!res.ok) return [];
  return res.json();
...
// 產生 sitemap 用：所有還看得到的貼文
export async function fetchAllPostIds(limit = 5000) {
  if (!SUPABASE_URL || !SUPABASE_ANON_KEY) return [];
  const res = await fetch(
    `${SUPABASE_URL}/rest/v1/posts?hidden=eq.false&order=id.desc&limit=${limit}&select=id,created_at`,
    { headers: { apikey: SUPABASE_ANON_KEY } }
  );
  if (!res.ok) return [];
  return res.json();

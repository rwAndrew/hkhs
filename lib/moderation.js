...
// 程式直接判斷、不送 AI 的：班級座號與明寫的學號。格式固定，用程式抓比 AI 穩。
//
// 比對前先把各種寫法攤平，不然「2O6-18」（字母 O）、「206 - 18」、「二零六 十八號」
// 都能繞過（2026/09/18 資安檢查實測）：
//   全形轉半形、中文數字轉阿拉伯數字、數字之間的字母 O／I／l 視為 0／1、
//   數字之間的空白與各種分隔符號一律拿掉
const CJK_DIGITS = { "零": 0, "〇": 0, "一": 1, "二": 2, "三": 3, "四": 4, "五": 5, "六": 6, "七": 7, "八": 8, "九": 9 };

// 把一串中文數字轉成阿拉伯數字：「二零六」→206（逐字）、「十八」→18、「二十一」→21（有「十」就當數值）
function cjkRunToDigits(run) {
  if (!run.includes("十")) return [...run].map((c) => CJK_DIGITS[c]).join("");
  const [tens, ones] = run.split("十");
  const t = tens === "" ? 1 : (CJK_DIGITS[tens] ?? NaN);
  const o = ones === "" ? 0 : (CJK_DIGITS[ones] ?? NaN);
  if (Number.isNaN(t) || Number.isNaN(o)) return run;   // 「二十三十」這種怪東西就別碰
  return String(t * 10 + o);
}

export function normalizeForPrefilter(text) {
  let t = String(text).normalize("NFKC");
  // 中文數字只在有「班」「號」這類座號脈絡時才轉，不然「一二三四五」這種
  // 隨口數數也會變成 12345 而誤擋
  if (/[班號]/.test(t)) t = t.replace(/[零〇一二三四五六七八九十]+/g, cjkRunToDigits);
  // 數字之間夾的 O/o → 0、I/l → 1（「2O6」「1l2」）；連續跑兩次處理 2OO6 這種
  for (let i = 0; i < 2; i++) t = t.replace(/(\d)[Oo](?=\d)/g, "$10").replace(/(\d)[Il](?=\d)/g, "$11").replace(/(?<=\d)[Oo](\d)/g, "0$1").replace(/(?<=\d)[Il](\d)/g, "1$1");
  // 數字之間的空白、點、破折號等分隔符拿掉：「206 - 18」→「20618」，「206.18」→「20618」
  t = t.replace(/(\d)[\s.·・,，、\-_—–~～]+(?=\d)/g, "$1");
  return t;
}

export function prefilter(text) {
  const t = normalizeForPrefilter(text);
  // 年級 1-3＋兩位班級＋座號 01-50，允許中間有「班」、結尾有「號」
  const seat = t.match(/(?<!\d)[1-3]\d{2}班?(?:0[1-9]|[1-4]\d|50)號?(?!\d)/);
  if (seat) return { verdict: "block", matched: seat[0], reason: "班級座號", model: "prefilter" };
  const sid = t.match(/學號\s*[:：]?\s*\d{4,}/);
